...
//! ## Features
//!
//! - **Order Building**: Create limit and market orders with proper rounding
//! - **Order Signing**: EIP-712 signature generation for order submission
//! - **Contract Configuration**: Polygon mainnet contract addresses
//!
//! ## Example
//!
//! ```rust,ignore
//! use polymarket_sdk::orders::{OrderBuilder, SigType};
//! use alloy_signer_local::PrivateKeySigner;
//!
//! let signer: PrivateKeySigner = "0x...".parse()?;
//! let builder = OrderBuilder::new(signer, None, None);
//!
//! let order = builder.create_order(
//!     137,  // chain_id
//!     &order_args,
//!     0,    // expiration
//!     &extras,
...
use crate::core::{OrderErrorKind, PolymarketError, Result};
use crate::types::{
    BookLevel, ExtraOrderArgs, MarketOrderArgs, OrderOptions, Side, SignedOrderRequest,
};
...
/// Contract configuration for a specific chain/risk setting
#[derive(Debug, Clone)]
...
/// # Returns
///
/// Contract configuration if supported, None otherwise
#[must_use]
...
fn decimal_to_token_units(amount: Decimal) -> Result<u64> {
    if amount < Decimal::ZERO {
        return Err(PolymarketError::order(
...
impl OrderBuilder {
    ...
    /// * `funder` - Funder/maker address (defaults to signer address)
    #[must_use]
    pub fn new(
        signer: PrivateKeySigner,
    ...
    /// # Arguments
    ///
    /// * `positions` - Order book levels to match against
    /// * `amount_to_match` - Amount to match in the order book
    ///
    /// # Returns
    ///
    /// Price at which the order would be filled
    ...
    /// * `order_args` - Market order arguments
    /// * `price` - Market price (calculated from order book)
    /// * `extras` - Extra order arguments
    ...
    pub fn create_market_order(
        ...
        let tick_size = options
            .tick_size
            .ok_or_else(|| PolymarketError::validation("Cannot create order without tick size"))?;
        ...
        let neg_risk = options
            .neg_risk
            .ok_or_else(|| PolymarketError::validation("Cannot create order without neg_risk"))?;

        let contract_config = get_contract_config(chain_id, neg_risk).ok_or_else(|| {
            PolymarketError::config("No contract found with given chain_id and neg_risk")
        })?;

        let exchange_address = Address::from_str(&contract_config.exchange)
            .map_err(|e| PolymarketError::config(format!("Invalid exchange address: {}", e)))?;
    ...
    ///
    /// * `chain_id` - Blockchain chain ID
    /// * `order_args` - Order arguments (token_id, side, price, size)
    ...
    fn build_signed_order(
        ...
        let taker_address = Address::from_str(&extras.taker)
            .map_err(|e| PolymarketError::validation(format!("Invalid taker address: {}", e)))?;

        let u256_token_id = U256::from_str_radix(&token_id, 10)
            .map_err(|e| PolymarketError::validation(format!("Incorrect tokenId format: {}", e)))?;
